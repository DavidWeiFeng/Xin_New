package controlPanel_fla
{
   import flash.display.MovieClip;
   
   [Embed(source="/_assets/assets.swf", symbol="symbol1149")]
   public dynamic class Timeline_11 extends MovieClip
   {
      
      public var flying_light:Icon_PetType_31;
      
      public function Timeline_11()
      {
         addFrameScript(0,this.frame1,7,this.frame8,8,this.frame9,10,this.frame11,11,this.frame12,35,this.frame36,39,this.frame40,43,this.frame44,47,this.frame48,51,this.frame52,55,this.frame56,58,this.frame59,61,this.frame62,64,this.frame65,67,this.frame68,70,this.frame71,73,this.frame74,76,this.frame77,79,this.frame80,82,this.frame83,85,this.frame86,91,this.frame92,94,this.frame95,97,this.frame98,100,this.frame101,103,this.frame104,106,this.frame107,109,this.frame110,112,this.frame113,115,this.frame116,118,this.frame119,121,this.frame122,124,this.frame125,127,this.frame128,130,this.frame131,133,this.frame134,136,this.frame137,139,this.frame140,142,this.frame143,145,this.frame146,148,this.frame149,151,this.frame152,154,this.frame155,157,this.frame158,160,this.frame161,163,this.frame164,166,this.frame167,169,this.frame170,172,this.frame173,175,this.frame176,178,this.frame179,181,this.frame182,184,this.frame185,187,this.frame188,190,this.frame191,193,this.frame194,196,this.frame197,199,this.frame200
         ,202,this.frame203,205,this.frame206,208,this.frame209,211,this.frame212,214,this.frame215,217,this.frame218,220,this.frame221,223,this.frame224,226,this.frame227,229,this.frame230,232,this.frame233,235,this.frame236,238,this.frame239,241,this.frame242,244,this.frame245,247,this.frame248,250,this.frame251,253,this.frame254,256,this.frame257,259,this.frame260,262,this.frame263,265,this.frame266,268,this.frame269,271,this.frame272,274,this.frame275,277,this.frame278,280,this.frame281,283,this.frame284,286,this.frame287,289,this.frame290,292,this.frame293,295,this.frame296,298,this.frame299,301,this.frame302,304,this.frame305,307,this.frame308,310,this.frame311,313,this.frame314,316,this.frame317,319,this.frame320,322,this.frame323,325,this.frame326,328,this.frame329,331,this.frame332,334,this.frame335,337,this.frame338,340,this.frame341,343,this.frame344,346,this.frame347,349,this.frame350,352,this.frame353,355,this.frame356,358,this.frame359,361,this.frame362,364,this.frame365,367,this.frame368
         ,370,this.frame371,373,this.frame374,376,this.frame377,379,this.frame380,382,this.frame383,385,this.frame386,388,this.frame389,391,this.frame392,394,this.frame395,397,this.frame398,400,this.frame401,403,this.frame404,406,this.frame407,409,this.frame410,412,this.frame413);
         super();
      }
      
      internal function frame1() : *
      {
         stop();
      }
      
      internal function frame8() : *
      {
         stop();
      }
      
      internal function frame9() : *
      {
         stop();
      }
      
      internal function frame11() : *
      {
         stop();
      }
      
      internal function frame12() : *
      {
         stop();
      }
      
      internal function frame36() : *
      {
         stop();
      }
      
      internal function frame40() : *
      {
         stop();
      }
      
      internal function frame44() : *
      {
         stop();
      }
      
      internal function frame48() : *
      {
         stop();
      }
      
      internal function frame52() : *
      {
         stop();
      }
      
      internal function frame56() : *
      {
         stop();
      }
      
      internal function frame59() : *
      {
         stop();
      }
      
      internal function frame62() : *
      {
         stop();
      }
      
      internal function frame65() : *
      {
         stop();
      }
      
      internal function frame68() : *
      {
         stop();
      }
      
      internal function frame71() : *
      {
         stop();
      }
      
      internal function frame74() : *
      {
         stop();
      }
      
      internal function frame77() : *
      {
         stop();
      }
      
      internal function frame80() : *
      {
         stop();
      }
      
      internal function frame83() : *
      {
         stop();
      }
      
      internal function frame86() : *
      {
         stop();
      }
      
      internal function frame92() : *
      {
         stop();
      }
      
      internal function frame95() : *
      {
         stop();
      }
      
      internal function frame98() : *
      {
         stop();
      }
      
      internal function frame101() : *
      {
         stop();
      }
      
      internal function frame104() : *
      {
         stop();
      }
      
      internal function frame107() : *
      {
         stop();
      }
      
      internal function frame110() : *
      {
         stop();
      }
      
      internal function frame113() : *
      {
         stop();
      }
      
      internal function frame116() : *
      {
         stop();
      }
      
      internal function frame119() : *
      {
         stop();
      }
      
      internal function frame122() : *
      {
         stop();
      }
      
      internal function frame125() : *
      {
         stop();
      }
      
      internal function frame128() : *
      {
         stop();
      }
      
      internal function frame131() : *
      {
         stop();
      }
      
      internal function frame134() : *
      {
         stop();
      }
      
      internal function frame137() : *
      {
         stop();
      }
      
      internal function frame140() : *
      {
         stop();
      }
      
      internal function frame143() : *
      {
         stop();
      }
      
      internal function frame146() : *
      {
         stop();
      }
      
      internal function frame149() : *
      {
         stop();
      }
      
      internal function frame152() : *
      {
         stop();
      }
      
      internal function frame155() : *
      {
         stop();
      }
      
      internal function frame158() : *
      {
         stop();
      }
      
      internal function frame161() : *
      {
         stop();
      }
      
      internal function frame164() : *
      {
         stop();
      }
      
      internal function frame167() : *
      {
         stop();
      }
      
      internal function frame170() : *
      {
         stop();
      }
      
      internal function frame173() : *
      {
         stop();
      }
      
      internal function frame176() : *
      {
         stop();
      }
      
      internal function frame179() : *
      {
         stop();
      }
      
      internal function frame182() : *
      {
         stop();
      }
      
      internal function frame185() : *
      {
         stop();
      }
      
      internal function frame188() : *
      {
         stop();
      }
      
      internal function frame191() : *
      {
         stop();
      }
      
      internal function frame194() : *
      {
         stop();
      }
      
      internal function frame197() : *
      {
         stop();
      }
      
      internal function frame200() : *
      {
         stop();
      }
      
      internal function frame203() : *
      {
         stop();
      }
      
      internal function frame206() : *
      {
         stop();
      }
      
      internal function frame209() : *
      {
         stop();
      }
      
      internal function frame212() : *
      {
         stop();
      }
      
      internal function frame215() : *
      {
         stop();
      }
      
      internal function frame218() : *
      {
         stop();
      }
      
      internal function frame221() : *
      {
         stop();
      }
      
      internal function frame224() : *
      {
         stop();
      }
      
      internal function frame227() : *
      {
         stop();
      }
      
      internal function frame230() : *
      {
         stop();
      }
      
      internal function frame233() : *
      {
         stop();
      }
      
      internal function frame236() : *
      {
         stop();
      }
      
      internal function frame239() : *
      {
         stop();
      }
      
      internal function frame242() : *
      {
         stop();
      }
      
      internal function frame245() : *
      {
         stop();
      }
      
      internal function frame248() : *
      {
         stop();
      }
      
      internal function frame251() : *
      {
         stop();
      }
      
      internal function frame254() : *
      {
         stop();
      }
      
      internal function frame257() : *
      {
         stop();
      }
      
      internal function frame260() : *
      {
         stop();
      }
      
      internal function frame263() : *
      {
         stop();
      }
      
      internal function frame266() : *
      {
         stop();
      }
      
      internal function frame269() : *
      {
         stop();
      }
      
      internal function frame272() : *
      {
         stop();
      }
      
      internal function frame275() : *
      {
         stop();
      }
      
      internal function frame278() : *
      {
         stop();
      }
      
      internal function frame281() : *
      {
         stop();
      }
      
      internal function frame284() : *
      {
         stop();
      }
      
      internal function frame287() : *
      {
         stop();
      }
      
      internal function frame290() : *
      {
         stop();
      }
      
      internal function frame293() : *
      {
         stop();
      }
      
      internal function frame296() : *
      {
         stop();
      }
      
      internal function frame299() : *
      {
         stop();
      }
      
      internal function frame302() : *
      {
         stop();
      }
      
      internal function frame305() : *
      {
         stop();
      }
      
      internal function frame308() : *
      {
         stop();
      }
      
      internal function frame311() : *
      {
         stop();
      }
      
      internal function frame314() : *
      {
         stop();
      }
      
      internal function frame317() : *
      {
         stop();
      }
      
      internal function frame320() : *
      {
         stop();
      }
      
      internal function frame323() : *
      {
         stop();
      }
      
      internal function frame326() : *
      {
         stop();
      }
      
      internal function frame329() : *
      {
         stop();
      }
      
      internal function frame332() : *
      {
         stop();
      }
      
      internal function frame335() : *
      {
         stop();
      }
      
      internal function frame338() : *
      {
         stop();
      }
      
      internal function frame341() : *
      {
         stop();
      }
      
      internal function frame344() : *
      {
         stop();
      }
      
      internal function frame347() : *
      {
         stop();
      }
      
      internal function frame350() : *
      {
         stop();
      }
      
      internal function frame353() : *
      {
         stop();
      }
      
      internal function frame356() : *
      {
         stop();
      }
      
      internal function frame359() : *
      {
         stop();
      }
      
      internal function frame362() : *
      {
         stop();
      }
      
      internal function frame365() : *
      {
         stop();
      }
      
      internal function frame368() : *
      {
         stop();
      }
      
      internal function frame371() : *
      {
         stop();
      }
      
      internal function frame374() : *
      {
         stop();
      }
      
      internal function frame377() : *
      {
         stop();
      }
      
      internal function frame380() : *
      {
         stop();
      }
      
      internal function frame383() : *
      {
         stop();
      }
      
      internal function frame386() : *
      {
         stop();
      }
      
      internal function frame389() : *
      {
         stop();
      }
      
      internal function frame392() : *
      {
         stop();
      }
      
      internal function frame395() : *
      {
         stop();
      }
      
      internal function frame398() : *
      {
         stop();
      }
      
      internal function frame401() : *
      {
         stop();
      }
      
      internal function frame404() : *
      {
         stop();
      }
      
      internal function frame407() : *
      {
         stop();
      }
      
      internal function frame410() : *
      {
         stop();
      }
      
      internal function frame413() : *
      {
         stop();
      }
   }
}

