package com.robot.core.manager
{
   ...
   public class FitmentManager
   {
      ...
      public static function saveInfo() : void
      {
         ...
         _local_3.writeUnsignedInt(roomID);
         ...
         SocketConnection.send(CommandID.SET_FITMENT,_local_3);
      }
      
      public static function saveRoomType(info:FitmentInfo, event:Function) : void
      {
         ...
         SocketConnection.addCmdListener(CommandID.SET_FITMENT,function(_arg_1:SocketEvent):void
         {
      ...
      public static function getUsedInfo(mapID:uint) : void
      {
         SocketConnection.addCmdListener(CommandID.FITMENT_USERING,function(_arg_1:SocketEvent):void
         {
            var _local_3:FitmentInfo = null;
            ...
            userID = _local_6.readUnsignedInt();
            roomID = _local_6.readUnsignedInt();
            var _local_7:uint = _local_6.readUnsignedInt();
            while(_local_8 < _local_7)
            {
               _local_3 = new FitmentInfo();
      ...
      {
         var _local_2:FitmentInfo = new FitmentInfo();
      ...
      {
         SocketConnection.addCmdListener(CommandID.FITMENT_ALL,function(_arg_1:SocketEvent):void
         {
            var _local_3:FitmentInfo = null;
            var _local_6:int = 0;
         ...
         if(t == SolidType.SUPER)
         {
            if(MainManager.actorInfo.vip == 1)
            {
               return data.filter(function(_arg_1:FitmentInfo, _arg_2:int, _arg_3:Array):Boolean
               {
                  if(_arg_1.unUsedCount > 0)
                  {
                     if(ItemXMLInfo.getIsSuper(_arg_1.id))
                     {
                        return true;
                     }
                  }
