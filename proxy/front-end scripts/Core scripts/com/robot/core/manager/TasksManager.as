package com.robot.core.manager
{
   ...
   public class TasksManager
   {
      ...
      public static function isParentAccept(id:uint, event:Function) : void
      {
         ...
         trace("len:" + len,JSON.stringify(arr),id);
         if(len == 0)
         {
            event(true);
            return;
      ...
      public static function parseTasks(data:ByteArray) : void
      {
         var b:uint = 0;
         var i:uint = 0;
         var bitOffset:uint = 0;
         var status:uint = 0;
         trace("解析任务数据",data.bytesAvailable);
         data.position = 0;
         var _local_11:int = 0;
         while(_local_11 < 1000)
         {
            b = data.readUnsignedByte();
            for(i = 0; i < 4; i++)
            {
               bitOffset = i * 2;
               status = uint(b >> bitOffset & 3);
               switch(status)
               {
                  case ALR_ACCEPT:
                     taskList.push(ALR_ACCEPT);
                     break;
                  case COMPLETE:
                     taskList.push(COMPLETE);
                     break;
                  default:
                     taskList.push(UN_ACCEPT);
               }
            }
            _local_11++;
         }
