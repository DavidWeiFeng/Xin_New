package com.robot.app.mapProcess
{
   ...
   public class MapProcess_42 extends BaseMapProcess
   {
      ...
      override protected function init() : void
      {
         ...
         var _local_2:PetModel = _local_1.pet;
         if(Boolean(_local_2))
         {
            if(PetXMLInfo.getType(_local_2.info.petID) == "16")
            {
               this.sc = 1.5;
            }
         }
      ...
      private function onPetShow(_arg_1:SocketEvent) : void
      {
         ...
         if(_local_2.userID == MainManager.actorInfo.userID)
         {
            if(_local_2.flag == 1)
            {
               if(PetXMLInfo.getType(_local_2.petID) == "16")
               {
            ...
            else if(PetXMLInfo.getType(_local_2.petID) == "16")
            {
      ...
      private function cheak() : void
      {
         ...
         {
            TasksManager.getProStatusList(8,function(_arg_1:Array):void
            {
               if(!_arg_1[3])
               {
                  _dia_1.visible = true;
      ...
      private function addBg() : void
      {
         ...
         var _local_1:int = int(LevelManager.root.getChildIndex(LevelManager.mapLevel));
         LevelManager.root.addChildAt(this.bg,_local_1);
         DisplayUtil.removeForParent(this.shape);
         this.shape = null;
         _local_1 = int(LevelManager.root.getChildIndex(LevelManager.toolsLevel));
         LevelManager.root.addChildAt(MapManager.currentMap.spaceLevel,_local_1 - 1);
      }
      ...
      {
         TasksManager.complete(8,3,function(_arg_1:Boolean):void
         {
      ...
      {
         TasksManager.complete(8,4,function(_arg_1:Boolean):void
         {
