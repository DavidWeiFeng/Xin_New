package com.robot.app.mapProcess
{
   ...
   public class MapProcess_56 extends BaseMapProcess
   {
      ...
      private function onGetCountHandler(_arg_1:SocketEvent) : void
      {
         ...
         this._remainTime = 5 - _local_2.miningCount;
         if(MainManager.actorInfo.superNono)
         {
            this._npc = NpcTipDialog.NONO;
         }
      ...
      private function onLightClickHandler(_arg_1:MouseEvent) : void
      {
         if(!this._appModel)
         {
      ...
      private function onWendyClickHandler(_arg_1:MouseEvent) : void
      {
         ...
         if(Boolean(MainManager.actorModel.nono) && Boolean(MainManager.actorModel.pet))
         {
            ...
            _local_2 = PetXMLInfo.getType(this._petInfo.petID);
            if(_local_2 == "7")
            {
      ...
      private function onGetExpHandler(_arg_1:SocketEvent) : void
      {
         ...
         if(_local_3 < 1000)
         {
            NpcTipDialog.show("恭喜哟，经过大陆架旋风的作用" + _local_5 + "增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 1000 && _local_3 < 2000)
         {
            NpcTipDialog.show("埋进大陆架旋风，" + _local_5 + "是最牛的精灵宝宝，吸收了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 2000 && _local_3 < 3000)
         {
            NpcTipDialog.show(" -_- 旋风威力~~~ " + _local_5 + "增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 3000)
         {
            NpcTipDialog.show("大陆架能量旋风果然了不得，" + _local_5 + "完美的吸收能量池的能量，增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
      }
      ...
      private function onPlantClickHandler(_arg_1:MouseEvent) : void
      {
      ...
      private function onNonoEnterHandler(_arg_1:Event) : void
      {
      ...
      private function onGetSuccessHandler(_arg_1:SocketEvent) : void
      {
         --this._remainTime;
      ...
      private function onGetStoHandler() : void
      {
         ...
         ItemInBagAlert.show(400026,_local_1 + "块" + TextFormatUtil.getRedTxt("希欧珊瑚") + "已经放入你的储存箱中！");
