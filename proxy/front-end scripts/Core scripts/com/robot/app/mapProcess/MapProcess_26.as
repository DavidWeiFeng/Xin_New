package com.robot.app.mapProcess
{
   ...
   public class MapProcess_26 extends BaseMapProcess
   {
      ...
      private function onAimatEnd(_arg_1:AimatEvent) : void
      {
         ...
         {
            if(this._beeMc.hitTestPoint(_local_2.endPos.x,_local_2.endPos.y))
            {
      ...
      private function onBeeClick(_arg_1:MouseEvent) : void
      {
         ...
         MainManager.actorModel.walkAction(new Point(_arg_1.stageX,_arg_1.stageY));
      }
      ...
      {
         var _local_1:Array = MainManager.actorInfo.clothIDs;
         var _local_2:Boolean = Boolean(ArrayUtil.embody(MainManager.actorInfo.clothIDs,[100048,100049,100050,100051]));
         if(!_local_2)
         {
      ...
      private function onCloudMcClickHandler(_arg_1:MouseEvent) : void
      {
         var _local_2:String = null;
         var _local_3:ActorModel = null;
         var _local_4:PetModel = null;
         var _local_5:uint = 0;
         ...
         {
            _local_2 = "你还没有获取" + TextFormatUtil.getRedTxt("幽浮捉迷藏") + "的每日任务呢，" + "快点击右上角的" + TextFormatUtil.getRedTxt("骄阳计划每日任务") + "按钮看看吧！";
            Alarm.show(_local_2);
         }
         else
         {
            _local_3 = MainManager.actorModel;
            _local_4 = _local_3.pet;
            if(Boolean(_local_4))
            {
               _local_5 = uint(_local_4.info.petID);
               if(_local_5 == 25 || _local_5 == 26)
               {
                  this.startHideGame();
               }
               else
      ...
      private function onRedClickHandler(_arg_1:MouseEvent) : void
      {
         conLevel["shipMc"]["redMc"].removeEventListener(MouseEvent.CLICK,this.onRedClickHandler);
         conLevel["shipMc"]["greenMc"].addEventListener(MouseEvent.CLICK,this.onGreenClickHandler);
      ...
      private function onGreenClickHandler(_arg_1:MouseEvent) : void
      {
         conLevel["shipMc"]["redMc"].addEventListener(MouseEvent.CLICK,this.onRedClickHandler);
         conLevel["shipMc"]["greenMc"].removeEventListener(MouseEvent.CLICK,this.onGreenClickHandler);
