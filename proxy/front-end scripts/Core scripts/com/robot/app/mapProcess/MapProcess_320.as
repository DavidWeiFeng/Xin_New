package com.robot.app.mapProcess
{
   ...
   public class MapProcess_320 extends BaseMapProcess
   {
      ...
      override protected function init() : void
      {
         ...
         if(TasksManager.getTaskStatus(42) == TasksManager.ALR_ACCEPT)
         {
            TasksManager.getProStatusList(42,function(_arg_1:Array):void
            {
               ...
               else if(Boolean(_arg_1[1]) && !_arg_1[2])
               {
                  state = "";
      ...
      private function onLeiYiClickHandler(_arg_1:MouseEvent) : void
      {
      ...
      private function onCloseFightBOSS(e:PetFightEvent) : void
      {
         ...
         {
            NpcTipDialog.show("我只是一个星球旅行者。\r(侠客应该已经成功将卡塔精灵护送到博士那里了，我们快赶回骄阳计划吧！)",function():void
            {
      ...
      private function onBailunMCClickHandler(_arg_1:MouseEvent) : void
      {
      }
      
      private function onLoadMovie2(_arg_1:MCLoadEvent) : void
      {
      }
      ...
      private function onFightMachineHandler(_arg_1:MouseEvent) : void
      {
         FightInviteManager.fightWithBoss("赫卡特",1);
         EventManager.addEventListener(PetFightEvent.FIGHT_RESULT,this.onCloseFight(_arg_1.currentTarget as MovieClip));
      }
      
      private function onCloseFight(mc:MovieClip) : Function
      {
         var func:Function = function(_arg_1:PetFightEvent):void
         {
         };
         return func;
      }
      
      private function onPartMCClickHandler(_arg_1:MouseEvent) : void
      {
      ...
      private function onMissileMCClickHandler(_arg_1:MouseEvent) : void
      {
         if(!this._panel)
         {
      ...
      private function onCanSendHandler(_arg_1:Event) : void
      {
      ...
      private function electriced(obj:Sprite) : void
      {
         ...
         timer1.addEventListener(TimerEvent.TIMER_COMPLETE,function(_arg_1:TimerEvent):void
         {
            var _local_3:MovieClip = null;
            timer1.removeEventListener(TimerEvent.TIMER_COMPLETE,arguments.callee);
            if(obj is BasePeoleModel)
            {
               _local_3 = BasePeoleModel(obj).skeleton.getSkeletonMC();
               _local_3.filters = null;
            }
            else
            {
               obj.filters = null;
      ...
      private function onFrameHandler1(_arg_1:Event) : void
      {
         ...
         if(Boolean(this.electricHead.getChildByName("electric_mc") as MovieClip))
         {
            if(Boolean((this.electricHead.getChildByName("electric_mc") as MovieClip).getChildByName("mc2")))
            {
               if((this.electricHead.getChildByName("electric_mc") as MovieClip).getChildByName("mc2").hitTestObject(MainManager.actorModel.sprite))
               {
                  ...
                  _local_2 = MapLibManager.getSound("electricSound");
                  _local_2.play();
                  return;
               }
            }
      ...
      private function onClickSprite(_arg_1:MouseEvent) : void
      {
      ...
      private function onMachineHandFrame(_arg_1:Event) : void
      {
         if(this.machineHand1.currentFrame == this.machineHand1.totalFrames)
         {
      ...
      {
         var _local_1:MCLoader = new MCLoader("resource/bounsMovie/down.swf",LevelManager.appLevel,1,"哇！不好！掉入？？？");
         _local_1.addEventListener(MCLoadEvent.SUCCESS,this.onLoadMovie);
         _local_1.doLoad();
      }
      ...
      private function onLoadMovie(event:MCLoadEvent) : void
      {
         ...
         MapManager.changeMap(35);
         content.addEventListener("EFFECT_END",function(_arg_1:Event):void
         {
