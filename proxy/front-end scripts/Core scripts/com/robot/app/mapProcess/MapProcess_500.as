package com.robot.app.mapProcess
{
   ...
   public class MapProcess_500 extends BaseMapProcess
   {
      ...
      {
         var bossClip:MovieClip = null;
         var bossPane:MLoadPane = null;
         var bossId:int = 0;
         ...
         for(var bossIndex:int = 0; bossIndex < this._allBossA.length; bossIndex++)
         {
            bossClip = this._allBossA[bossIndex] as MovieClip;
            bossPane = new MLoadPane(bossClip);
            bossPane.fitType = bossClip.width > bossClip.height ? uint(MLoadPane.FIT_WIDTH) : uint(MLoadPane.FIT_HEIGHT);
            bossPane.setSizeWH(80,80);
            this._bossContainer.addChild(bossPane);
            bossPane.x = 100 * bossIndex;
            bossId = int(FightLevelModel.getBossId[bossIndex]);
            bossName = PetXMLInfo.getName(bossId);
            ToolTipManager.add(bossPane,bossName);
         }
