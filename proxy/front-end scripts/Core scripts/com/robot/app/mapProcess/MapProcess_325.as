package com.robot.app.mapProcess
{
   ...
   public class MapProcess_325 extends BaseMapProcess
   {
      ...
      private function screenPlay(_arg_1:MouseEvent) : void
      {
         conLevel["shortTree"].gotoAndStop(2);
      }
      
      private function screenOver(_arg_1:MouseEvent) : void
      {
         conLevel["shortTree"].gotoAndStop(1);
      }
      ...
      private function thinksFun(_arg_1:MouseEvent) : void
      {
      ...
      private function godOver(_arg_1:Event) : void
      {
         this.godSound = null;
      ...
      private function completeTwo(_arg_1:Event) : void
      {
         this.twoSound = null;
      ...
      private function musicComplete(_arg_1:Event) : void
      {
      ...
      private function isShowBoss(_arg_1:uint) : void
      {
         if(this.isBoss)
         {
            return;
         }
         if(this.musicIdArr[this.musicId] == _arg_1)
         {
            ++this.musicId;
            if(this.musicId == 7)
            {
      ...
      private function clickBossHandler(_arg_1:MouseEvent) : void
      {
      ...
      private function isPlayMusic(param1:uint) : void
      {
            ...
            if(this.musicNum == 3)
            {
               if(this.isInTask)
               {
                  ...
                  AnimateManager.playMcAnimate(depthLevel["jellyseer_mc"],2,"mc2",function():void
                  {
                     ...
                     {
                        NpcDialog.show(NPC.SEER,["带我去月影花园看看吧……那里也跟这里一样奇妙吗？也有音乐吗？"],["哎呀！哎呀！你别着急啊……"],[function():void
                        {
                           TasksManager.complete(TaskController_133.TASK_ID,1,function(_arg_1:Boolean):void
                           {
                              MapManager.changeMap(63);
                           });
                        }]);
         ...
         else
         {
            ...
            if(this.musicNum >= 3)
            {
               if(this.isInTask)
               {
                  NpcDialog.show(NPC.SEER,["我想你弹奏错了……再认真看下琴谱吧！#1（★★△△**△）"],["我再去试试看！"],[function():void
                  {
                     musicNum = 0;
                  }]);
               }
