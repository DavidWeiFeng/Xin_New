package com.robot.app.mapProcess
{
   ...
   public class MapProcess_59 extends BaseMapProcess
   {
      ...
      private function overHanderMC(_arg_1:MouseEvent) : void
      {
      ...
      private function outHandleMC(_arg_1:MouseEvent) : void
      {
         if(Boolean(this.numDoor["mc1"]))
         {
            this.numDoor["mc1"].stop();
      ...
      private function doorAddNumber(_arg_1:MouseEvent) : void
      {
         if(numberOfDoor >= 3)
         {
            numberOfDoor = 3;
      ...
      private function overHander(_arg_1:MouseEvent) : void
      {
         this.bigWater.gotoAndPlay(1);
      }
      
      private function outHander(_arg_1:MouseEvent) : void
      {
         this.bigWater.gotoAndStop(1);
      }
      ...
      private function moneyHave(_arg_1:MouseEvent) : void
      {
      ...
      private function onCountOK(_arg_1:Event) : void
      {
         dayOre.removeEventListener(DayOreCount.countOK,this.onCountOK);
         this.nonoActive();
      }
      
      private function nonoActive(_arg_1:MouseEvent = null) : void
      {
      ...
      private function onSuccess(_arg_1:SocketEvent) : void
      {
         ...
         {
            _local_2 = "2块";
            NpcTipDialog.show("飘呀飘呀！主人，我感觉我的头像电灯泡，你看我帅吗……嘿嘿！" + TextFormatUtil.getRedTxt(_local_2 + "尼古滴露") + "到手咯！",null,NpcTipDialog.NONO);
         }
         else
         {
            _local_2 = "1块";
            NpcTipDialog.show("铛铛铛……我拿到了" + TextFormatUtil.getRedTxt(_local_2 + "尼古滴露") + "哦！主人，你说我厉害不厉害呀！",null,NpcTipDialog.NONO);
         }
      }
      ...
      private function onLoadPetHandler(mc:DisplayObject) : void
      {
         ...
         {
            PetManager.storageUpDate(MainManager.actorModel.pet.info.catchTime,function(_arg_1:PetInfo):void
            {
               petLevel = _arg_1.level;
            });
         }
         if(Boolean(mc))
         {
            ...
            this.timeNum_2 = setTimeout(function():void
            {
               ...
               {
                  _local_1 = TextFormatUtil.getRedTxt(_petName);
                  NpcTipDialog.show(_local_1 + "是100级的精灵哟，没法再吸收能量长大了！",showUp,NpcTipDialog.NONO,0,showUp);
               }
            },3000);
         }
      }
      ...
      private function onGetExpHandler(_arg_1:SocketEvent) : void
      {
         ...
         var _local_4:String = TextFormatUtil.getRedTxt(String(_local_3));
         var _local_5:String = TextFormatUtil.getRedTxt(this._petName);
         if(_local_3 < 1000)
         {
            NpcTipDialog.show("嘿!这泉水果然与众不同呢，有点甜？有点酸？" + _local_5 + "增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 1000 && _local_3 < 2000)
         {
            NpcTipDialog.show("啦啦啦！继续淋浴尼古拉斯的泉水吧，" + _local_5 + "这可对你有好处呐！你已经吸收了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 2000 && _local_3 < 3000)
         {
            NpcTipDialog.show(" ( ⊙o⊙ )哇~~~  " + _local_5 + "增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
         if(_local_3 >= 3000)
         {
            NpcTipDialog.show("尼古拉斯湾的泉水果然与众不同，" + _local_5 + "经过了泉水的孕育，增长了" + _local_4 + "点经验。",this.showUp,NpcTipDialog.NONO,0,this.showUp);
         }
      }
