package com.robot.app.mapProcess
{
   ...
   public class MapProcess_60 extends BaseMapProcess
   {
      ...
      public function MapProcess_60()
      {
         ...
         TasksManager.getProStatusList(97,function(_arg_1:Array):void
         {
         ...
         {
            TasksManager.getProStatusList(83,function(_arg_1:Array):void
            {
               blackHot.visible = true;
               blackHot.gotoAndStop(1);
               if(_arg_1[0] && !_arg_1[1] || _arg_1[1] && !_arg_1[2])
               {
         ...
         TasksManager.getProStatusList(TaskController_79.TASK_ID,function(_arg_1:Array):void
         {
            if(!_arg_1[0])
            {
               TaskController_79.inScean();
            }
            else if(Boolean(_arg_1[0]))
            {
      ...
      private function pleaseEva(e:MouseEvent) : void
      {
         NpcDialog.show(NPC.EVA,["咦？#7伊娃伊娃!彭恰恰！！#6艾迪？GOGOGO~~~~"],["嘿嘿……"],[function():void
         {
            ...
            {
               _local_1 = null;
               _local_2 = "TaskPanel_97";
               if(_local_1)
               {
                  _local_1.destroy();
                  _local_1 = null;
               }
               _local_1 = new AppModel(ClientConfig.getTaskModule(_local_2),"正在打开任务信息");
               _local_1.setup();
               _local_1.show();
            }
         }]);
      ...
      private function gameWin(_arg_1:GamePlatformEvent) : void
      {
      }
      ...
      {
         var _local_1:uint = uint(Math.random() * 2);
         if(_local_1 == 0)
         {
      ...
      private function closeChoose(_arg_1:MouseEvent) : void
      {
         this.guess.visible = false;
         depthLevel.mouseEnabled = true;
         depthLevel.mouseChildren = true;
      ...
      private function chooseSec(_arg_1:MouseEvent) : void
      {
         var _local_2:uint = 0;
         while(_local_2 < 4)
         {
            this.guess["mc" + _local_2].gotoAndStop(1);
            _local_2++;
         }
      ...
      private function showSpeak(e:MouseEvent) : void
      {
         ...
         TasksManager.getProStatusList(83,function(_arg_1:Array):void
         {
            if(TasksManager.getTaskStatus(83) == TasksManager.ALR_ACCEPT && !_arg_1[0])
            {
            ...
            if(!arr[0])
            {
               if(numAdd == 0)
               {
                  NpcTipDialog.showAnswer("救命！救命！这下面困着一个精灵，它似乎奄奄一息了！不过下面可是很危险的，你愿意为精灵去冒生命危险吗？",function():void
                  {
                     ...
                     {
                        var _local_1:MCLoader = new MCLoader("resource/bounsMovie/downSeer.swf",LevelManager.appLevel,1,"正在打开赛尔掉落动画");
                        _local_1.addEventListener(MCLoadEvent.SUCCESS,onLoadControlMovie);
                        _local_1.doLoad();
                     },NpcTipDialog.SEER);
      ...
      private function movieOver(_arg_1:Event) : void
      {
      ...
      private function clickHamOHandler(_arg_1:Event) : void
      {
         TaskController_79.dongHua2();
      }
      
      private function clickLongHandler(_arg_1:MouseEvent) : void
      {
         if(TasksManager.getTaskStatus(TaskController_79.TASK_ID) == TasksManager.COMPLETE)
         {
      ...
      private function clickShiTouHandler(_arg_1:MouseEvent) : void
      {
