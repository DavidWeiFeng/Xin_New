package com.robot.app.sceneInteraction
{
   ...
   public class BailuenModel extends ActionSpriteModel implements IAimatSprite
   {
      ...
      public function set hp(_arg_1:uint) : void
      {
         ...
         this._bloodMc["maskMc"].width = _arg_1 / 100 * this._bloodMc["viswMc"].width;
         if(_arg_1 == 0)
         {
            this.setAction(DIE);
         }
      ...
      public function show(_arg_1:Point, _arg_2:Boolean) : void
      {
         if(Boolean(this._obj))
         {
            return;
      ...
      public function aimatState(_arg_1:AimatInfo) : void
      {
         if(this._currAction != 1)
         {
            return;
         }
         if(atype.indexOf(_arg_1.id) == -1)
         {
            return;
         }
         if(Boolean(this._aimatStateManager))
         {
            this._aimatStateManager.execute(_arg_1);
         }
         SocketConnection.send(CommandID.ATTACK_BAILUEN);
         this.setAction(HIT);
      }
      
      public function showBox(_arg_1:String, _arg_2:Number = 0) : void
      {
         if(Boolean(this._dialogBox))
         {
            this._dialogBox.destroy();
         ...
         this._dialogBox.show(_arg_1,0,-this.height + _arg_2,this);
      }
      ...
      private function setAction(_arg_1:int) : void
      {
         if(Boolean(this._obj))
         {
            this._obj.addEventListener(Event.ENTER_FRAME,this.onEnter);
            this._obj.gotoAndPlay(_arg_1);
            this._currAction = _arg_1;
         }
         stopAutoWalk();
      }
      ...
      private function onLoadPanel(_arg_1:MCLoadEvent) : void
      {
         this._load.removeEventListener(MCLoadEvent.SUCCESS,this.onLoadPanel);
         var _local_2:Class = Utils.getClassFromLoader("UI_BailuenPanel",this._load.loader);
         this._panel = new _local_2() as Sprite;
         if(Boolean(this._panel))
         {
            LevelManager.topLevel.addChild(this._panel);
            DisplayUtil.align(this._panel,null,AlignType.MIDDLE_CENTER);
