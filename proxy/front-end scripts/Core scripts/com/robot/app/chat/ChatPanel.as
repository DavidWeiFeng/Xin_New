package com.robot.app.chat
{
   ...
   public class ChatPanel extends Sprite
   {
      ...
      private function onChat(_arg_1:ChatEvent) : void
      {
         var _local_2:String = _arg_1.info.msg;
         if(_local_2.substr(0,1) == "#")
         {
            if(UIManager.hasDefinition("e" + _local_2.substring(1,_local_2.length)))
            {
               return;
            }
         }
         TextFormatUtil.appSenderFormatText(this._txt,_arg_1.info.senderNickName + ": ",false);
         TextFormatUtil.appDefaultFormatText(this._txt,_local_2 + "\n",0);
