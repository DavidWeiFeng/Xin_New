package com.robot.app.messagetool
{
   ...
   public class MessageTool
   {
      ...
      public static function start() : void
      {
         ...
         {
            TasksManager.getProStatus(TaskController_38.TASK_ID,1,function(b:Boolean):void
            {
               if(b)
               {
      ...
      public static function showMessagePanel(e:Event) : void
      {
         TasksManager.getProStatusList(TaskController_38.TASK_ID,function(_arg_1:Array):void
         {
            if(Boolean(_arg_1[1]) && !_arg_1[4])
            {
         ...
         TasksManager.getProStatusList(TaskController_38.TASK_ID,function(_arg_1:Array):void
         {
            if(Boolean(_arg_1[4]) && !_arg_1[8])
            {
               NpcTipDialog.show("在飞船上不要被周围的奇怪现象吓住，找出问题的核心。\n    继续努力哦！",null,NpcTipDialog.IRIS);
            }
         });
         TasksManager.getProStatus(TaskController_38.TASK_ID,8,function(_arg_1:Boolean):void
         {
            if(_arg_1)
            {
