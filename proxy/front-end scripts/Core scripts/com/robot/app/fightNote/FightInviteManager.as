package com.robot.app.fightNote
{
   ...
   public class FightInviteManager
   {
      ...
      private static var NUM:Number = 0.04;
      ...
      public static function fightWithPlayer(_arg_1:UserInfo) : void
      {
         ...
         FightWaitPanel.selectMode(_arg_1);
      }
      
      public static function fightWithNpc(_arg_1:uint) : void
      {
         if(!fightSwitch)
         {
            return;
         ...
         SocketConnection.send(CommandID.FIGHT_NPC_MONSTER,_arg_1);
         fightSwitch = false;
         changeSwitch();
      }
      
      public static function fightWithBoss(_arg_1:String, _arg_2:uint = 0, _arg_3:Boolean = false) : void
      {
         if(!fightSwitch)
         {
            return;
         }
         if(Math.random() < NUM && !AutomaticFightManager.isStart && !_arg_3)
         {
            KillPluginSys.start();
            return;
         }
         PetFightModel.enemyName = _arg_1;
         PetFightModel.mode = PetFightModel.MULTI_MODE;
         PetFightModel.status = PetFightModel.FIGHT_WITH_BOSS;
         SocketConnection.send(CommandID.CHALLENGE_BOSS,_arg_2);
         fightSwitch = false;
         changeSwitch();
      }
      ...
      public static function add(_arg_1:InviteNoteInfo) : void
      {
         array.push(_arg_1);
         checkLength();
      }
      ...
      {
         var _local_1:InviteNoteInfo = array.shift() as InviteNoteInfo;
         checkLength();
         return _local_1;
      }
      ...
      private static function showInviteAnswer(event:MouseEvent) : void
      {
         ...
         acceptInvite = function():void
         {
            ...
            PetFightModel.enemyID = data.userID;
