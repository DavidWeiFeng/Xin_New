package com.robot.app.buyItem
{
   ...
   public class ItemCmdListener extends BaseBeanController
   {
      ...
      private function onMultiItemBuy(_arg_1:SocketEvent) : void
      {
         var _local_2:BuyMultiItemInfo = _arg_1.data as BuyMultiItemInfo;
         Alarm.show("<font color=\'#FF0000\'>" + ITEM_NAME + "</font>已经放入你的储存箱！");
         MainManager.actorInfo.coins = _local_2.cash;
         EventManager.dispatchEvent(new Event(ItemAction.BUY_MUILTY));
      }
      
      private function onBuyGoldProduct(_arg_1:SocketEvent) : void
      {
         var _local_2:uint = 0;
         var _local_3:String = null;
         var _local_4:GoldBuyProductInfo = _arg_1.data as GoldBuyProductInfo;
         var _local_5:Array = [];
         var name:String = null;
         name = GoldProductXMLInfo.getNameByProID(ProductAction.productID);
         _local_5 = GoldProductXMLInfo.getItemIDs(ProductAction.productID);
         for each(_local_2 in _local_5)
         {
            _local_3 = ItemXMLInfo.getName(_local_2);
            if(_local_2 > 500000)
            {
               IconAlert.show("恭喜你购买成功，" + TextFormatUtil.getRedTxt(_local_3) + "已经放入你的基地仓库中",_local_2);
            }
            else
            {
               ItemInBagAlert.show(_local_2,"恭喜你购买成功，" + TextFormatUtil.getRedTxt(_local_3) + "已经放入你的储存箱中");
            }
         }
      }
      
      private function onBuyMoneyProduct(_arg_1:SocketEvent) : void
      {
         var _local_2:uint = 0;
         var _local_3:String = null;
         var _local_4:MoneyBuyProductInfo = _arg_1.data as MoneyBuyProductInfo;
         var _local_5:Array = MoneyProductXMLInfo.getItemIDs(ProductAction.productID);
         for each(_local_2 in _local_5)
         {
            _local_3 = ItemXMLInfo.getName(_local_2);
            if(_local_2 == 5)
            {
               _local_3 = MoneyProductXMLInfo.getNameByProID(ProductAction.productID);
               IconAlert.show("你获得了" + TextFormatUtil.getRedTxt(_local_3) + "，快去<font color=\'#ff0000\'>《宇宙购物指南》</font>中购物吧！^_^",_local_2);
            }
            else if(_local_2 > 500000)
            {
               IconAlert.show("恭喜你购买成功，" + TextFormatUtil.getRedTxt(_local_3) + "已经放入你的基地仓库中",_local_2);
            }
            else
