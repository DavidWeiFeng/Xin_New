package com.robot.app.energy.utils
{
   ...
   public class EnergyController
   {
      ...
      public static function exploit(_arg_1:uint = 0) : void
      {
      ...
      private static function onCountOK(_arg_1:Event) : void
      {
         ...
         var _local_2:String = "探测发现这块区域埋藏着能源" + tipStr + ",宝贵的能源" + tipStr + "可以给赛尔飞船提供能源。\n    你是否要" + type + "一些这样的" + tipStr + "呢？";
         NpcTipDialog.showAnswer(_local_2,exploitNow,null,NpcTipDialog.SHU_KE);
      }
      ...
      private static function exploitNow() : void
      {
         ...
         var _local_1:Sprite = MainManager.actorModel.sprite;
         _local_1.parent.addChild(_local_1);
         if(waitTime != null)
         {
      ...
      public static function showDayTalkInfo(info:DayTalkInfo) : void
      {
         var _cateInfo:CateInfo = null;
         var str:String = null;
         var nameStr:String = null;
         ...
         _cateInfo = null;
         var setStr:Function = function(itemInfo:CateInfo):void
         {
            switch(itemInfo.id)
            {
               case 400001:
            ...
            nameStr = ItemXMLInfo.getName(itemInfo.id);
         };
         if(info.outList.length > 0)
         {
            for(j = 0; j < info.outList.length; j++)
            {
               _cateInfo = info.outList[j];
               setStr(_cateInfo);
               alertStr = _cateInfo.count + ruleStr + "<font color=\'#ff0000\'>" + nameStr + "</font>已放入了你的储存箱。";
               ItemInBagAlert.show(_cateInfo.id,alertStr);
            }
         }
      }
   }
