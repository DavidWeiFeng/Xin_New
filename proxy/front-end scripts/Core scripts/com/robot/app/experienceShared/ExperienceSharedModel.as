package com.robot.app.experienceShared
{
   ...
   public class ExperienceSharedModel
   {
      ...
      private static function onGetMyExpCompleteHandler(_arg_1:SocketEvent) : void
      {
         SocketConnection.removeCmdListener(CommandID.GETMYEXPERIENCE_COMPLETE,onGetMyExpCompleteHandler);
         var _local_2:GetExperienceInfo = _arg_1.data as GetExperienceInfo;
         if(_local_2.getExp == 0)
         {
            Alarm.show("你还没有为学员积累经验，加油！");
         }
         else
         {
            Alarm.show("你累积了 " + _local_2.getExp + " 点经验还未被领取");
         }
      }
      ...
      private static function onGetCompleteHandler(_arg_1:SocketEvent) : void
      {
         SocketConnection.removeCmdListener(CommandID.MYEXPERIENCEPOND_COMPLETE,onGetCompleteHandler);
         var _local_2:MyExperiencePondInfo = _arg_1.data as MyExperiencePondInfo;
         var _local_3:uint = _local_2.getMyExp;
         Alert.show("教官已经积累经验值:" + _local_3 + "\n确定要平均分配给背包里的所有精灵吗?",getExp);
      }
      ...
      private static function onSendCompleteHandler(_arg_1:SocketEvent) : void
      {
         ...
         var _local_2:ExperienceSharedInfo = _arg_1.data as ExperienceSharedInfo;
         Alarm.show("你所有精灵共获得了经验:" + _local_2.getFraction);
      }
   }
}
