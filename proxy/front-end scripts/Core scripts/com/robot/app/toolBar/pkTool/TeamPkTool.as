package com.robot.app.toolBar.pkTool
{
   ...
   public class TeamPkTool
   {
      ...
      public function TeamPkTool()
      {
         ...
         for each(_local_1 in this.mouseIconList)
         {
            _local_1.addEventListener(MouseEvent.CLICK,this.clickIcon);
            this.box.append(new UIMovieClip(_local_1.sprite));
         }
      ...
      private function onToolHandler(_arg_1:TeamPKEvent) : void
      {
         if(_arg_1.type == TeamPKEvent.CLOSE_TOOL)
         {
            this.close();
         }
         else
      ...
      private function clickIcon(_arg_1:MouseEvent) : void
      {
         this.currentMouseIcon = _arg_1.currentTarget as IPKMouseIcon;
