package com.robot.app.aimat
{
   ...
   public class Aimat_3 extends BaseAimat
   {
      ...
      private function compSoundHandler(_arg_1:Event) : void
      {
      ...
      private function compSoundEndHandler(_arg_1:Event) : void
      {
      ...
      private function onEnter(_arg_1:Event) : void
      {
         ...
         if(Math.abs(this.ui.x - _info.endPos.x) < this._speed / 2 && Math.abs(this.ui.y - _info.endPos.y) < this._speed / 2)
         {
            ...
            this.ui2.rotation = this.ui.rotation;
            ...
            _local_2 = MapManager.getObjectsPointRect(_info.endPos,30,[IAimatSprite]);
            if(AimatXMLInfo.getIsStage(_info.id) != 0)
            {
            }
            for each(_local_3 in _local_2)
            {
               if(_local_3 is IAimatSprite)
               {
                  IAimatSprite(_local_3).aimatState(_info);
               }
            }
            return;
         }
