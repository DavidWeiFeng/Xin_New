package com.robot.app.aimat
{
   ...
   public class Aimat_10037 extends BaseAimat
   {
      ...
      private function enterFranepao(_arg_1:Event) : void
      {
         var _local_2:MovieClip = _arg_1.target as MovieClip;
         if(_local_2.currentFrame == _local_2.totalFrames - 1)
         {
            _local_2.removeEventListener(Event.ENTER_FRAME,this.enterFranepao);
            MapManager.currentMap.depthLevel.removeChild(_local_2);
         }
      }
      ...
      private function onEnter(_arg_1:Event) : void
      {
         ...
         if(this.timen == 2)
         {
            _local_2 = AimatController.getResEffect(_info.id,"_0");
            _local_2.x = this.ui.x;
            _local_2.y = this.ui.y;
            MapManager.currentMap.depthLevel.addChild(_local_2);
            _local_2.addEventListener(Event.ENTER_FRAME,this.enterFranepao);
            this.timen = 0;
         }
         if(Math.abs(this.ui.x - _info.endPos.x) < this._speed / 2 && Math.abs(this.ui.y - _info.endPos.y) < this._speed / 2)
         {
            ...
            _local_3 = MapManager.getObjectsPointRect(_info.endPos,30,[IAimatSprite]);
            for each(_local_4 in _local_3)
            {
               if(_local_4 is IAimatSprite)
               {
                  IAimatSprite(_local_4).aimatState(_info);
               }
            }
            return;
         }
