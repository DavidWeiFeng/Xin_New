   ...
   public class DailyTaskController
   {
      ...
      private static function showIcon() : void
      {
         if(!icon)
         {
            ...
            trace(uint(SOManager.getUserSO(SOManager.DAILY_TASK).data["ver"]),ClientConfig.dailyTask);
      ...
      private static function clickHandler(event:MouseEvent) : void
      {
         ...
         var so:SharedObject = SOManager.getUserSO(SOManager.DAILY_TASK);
         so.data["ver"] = ClientConfig.dailyTask;
         SOManager.flush(so);
      }
   }
}
