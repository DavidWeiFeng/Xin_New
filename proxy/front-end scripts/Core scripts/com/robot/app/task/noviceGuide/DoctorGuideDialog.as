package com.robot.app.task.noviceGuide
{
   ...
   public class DoctorGuideDialog
   {
      ...
      public static function showDialog(_arg_1:uint = 0) : void
      {
         npcMc = NpcTipDialog.DOCTOR;
         var _local_2:uint = uint(TasksManager.taskList[2]);
         if(_local_2 == 1 && !GuideTaskModel.bTaskDoctor)
         {
            NpcTipDialog.show("我是派特博士，研究精灵是我的工作。这里就是精灵实验室，这里很多神奇的设备都和精灵有关。还有关于精灵的问题？快打开精灵手册，了解下精灵的介绍吧！",okFun,npcMc,-80);
         }
         else if(_local_2 == 1 && GuideTaskModel.bReadMonBook)
         {
      ...
      private static function selGrass(_arg_1:MouseEvent) : void
      {
         SelectPet.checkStatus(1);
      }
      
      private static function selFireMC(_arg_1:MouseEvent) : void
      {
         SelectPet.checkStatus(2);
      }
      
      private static function selWaterMC(_arg_1:MouseEvent) : void
      {
