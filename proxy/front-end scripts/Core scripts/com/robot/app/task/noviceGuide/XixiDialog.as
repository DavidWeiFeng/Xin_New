package com.robot.app.task.noviceGuide
{
   ...
   import com.robot.app.task.noviceGuide.GuideTaskAfter.*;
   ...
   public class XixiDialog
   {
      ...
      public static function show(id:uint = 0) : void
      {
         ...
         if(id == 94)
         {
            TasksManager.getProStatusList(94,function(arr:Array):void
            {
               ...
               if(Boolean(arr[2]) && !arr[3])
               {
                  ...
                  {
                     ItemManager.removeEventListener(ItemEvent.COLLECTION_LIST,arguments.callee);
                     var _local_3:SingleItemInfo = ItemManager.getCollectionInfo(400002);
                     if(_local_3 != null && _local_3.itemNum >= 5)
                     {
         ...
         {
            NpcTipDialog.show("看到<font color=\'#FF0000\'>精灵包</font>里的那只精灵了吧，是不是有点激动。别着急，我先给你介绍下骄阳计划飞船，了解下我们远航的目的。",onReceive,xixi,-60);
            return;
         }
         if(!TasksManager.isComNoviceTask())
         {
            if(TasksManager.taskList[2] == 1)
            {
               ...
               {
                  NpcTipDialog.show("你已经从船长和博士那得到了不少信息吧，我现在把新手奖励送给你，现在你是骄阳计划飞船上的新丁了，希望你结交到很多的好朋友一起探险哦!",onSubmit,xixi,-60);
               }
               else
               {
      ...
      private static function onCheckSkeeTask(_arg_1:Array) : void
      {
         if(_arg_1[0] == true && _arg_1[1] == true)
         {
         }
      }
      ...
      public static function showNew() : void
      {
         ...
         if(!newPanel)
         {
            newPanel = CoreAssetsManager.getMovieClip("lib_newer_mc");
            DisplayUtil.align(newPanel,null,AlignType.MIDDLE_CENTER);
            btn = newPanel["goBtn"];
            btn.addEventListener(MouseEvent.CLICK,function(_arg_1:MouseEvent):void
            {
               getFirstMonster();
