   ...
   public class ConscribeTeam
   {
      ...
      private static function sayW() : void
      {
         TasksManager.complete(TASK_ID,2,function(_arg_1:Boolean):void
         {
            ...
            if(_arg_1)
            {
               _local_2 = "干得漂亮！恭喜你正式加入SPT先锋队,这些奖励是你应得的。";
               NpcTipDialog.show(_local_2,null,NpcTipDialog.CICI);
            }
            else
            {
               _local_2 = "还没有办完吗？加入SPT先锋队可是很光荣的哦！";
               NpcTipDialog.show(_local_2,null,NpcTipDialog.CICI);
            }
         });
      }
      ...
      private static function hasKillM() : void
      {
         ...
         {
            TasksManager.getProStatus(TASK_ID,1,function(b:Boolean):void
            {
               if(!b)
               {
      ...
      private static function onAccept(b:Boolean) : void
      {
         ...
         if(b)
         {
            ...
            isSayB = false;
            panDuan();
            str = "赛尔先锋队SPT是我们骄阳计划上的一个特殊组织，每一个有决心完成骄阳计划太空任务的船员都有机会加入先锋队。";
      ...
      private static function hasFile(_arg_1:ItemEvent) : void
      {
         var _local_2:String = null;
         var _local_4:int = 0;
         ItemManager.removeEventListener(ItemEvent.CLOTH_LIST,hasFile);
         var _local_3:Array = ItemManager.getClothIDs();
         while(_local_4 < _local_3.length)
         {
            if(_local_3[_local_4] == 100044)
            {
