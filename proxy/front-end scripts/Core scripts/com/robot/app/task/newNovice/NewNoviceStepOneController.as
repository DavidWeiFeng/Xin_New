package com.robot.app.task.newNovice
{
   ...
   public class NewNoviceStepOneController
   {
      ...
      private static var DIA_1_A:Array = ["我是飞船机械师茜茜，欢迎你登上骄阳计划飞船！看完了精彩的动画，对即将开始的探索之旅是不是很向往呢？","不过在这之前你需要对骄阳计划有个大致的了解，这样才能尽快适应在飞船上的生活！我已经安排好了一堂内容丰富的课程。事不宜迟，现在就让我们开始吧！","在骄阳计划内走动是很简单的，鼠标左键点击一下空地，就能移动到该位置了，当然有些地方是不可行走的！","与Npc对话也是一样，比如你以后要找我聊天时，到机械室点我就能进行交流了！","现在茜茜会为你介绍一下我们通常会使用到的一些功能。请注意看下方功能栏中最左边的选项，这是快捷语言栏，可以方便你进行一些简单的交流。","这是表情栏，你的喜怒哀乐，用适当的表情来体现，会让身边的朋友感受更深刻哦！","这是动作栏，当你穿齐一些特殊的装备时，可以做出一些特别的动作，甚至改变自己的形态！","这是瞄准栏，我们赛尔拥有头部射击的能力，这是星球中探索不可缺少的！配备不同的头具时，我们能发出的头部射击效果也会有所改变。另外一些特殊的投掷道具也会存放在这里。","嘿嘿，看你听的这么认真！嗯……好吧！我身边这套闪闪发亮的装备就送给你咯！快用鼠标左键去点它，把它收入你的储存箱中吧！","嘿嘿，看你听的这么认真！嗯……好吧！我身边这套闪闪发亮的装备就送给你咯！快用鼠标左键去点它，把它收入你的储存箱中吧！"];
      ...
      public static function start() : void
      {
         ...
         switch(stu)
         {
            case TasksManager.UN_ACCEPT:
               TasksManager.accept(STEP_ID,function(_arg_1:Boolean):void
               {
                  if(_arg_1)
                  {
      ...
      public static function playAnimation(_arg_1:String) : void
      {
         _curPath = _arg_1;
         _mcLoader = new MCLoader(_arg_1,LevelManager.topLevel,1,"正在打开动画");
      ...
      private static function onLoad(_arg_1:MCLoadEvent) : void
      {
         ...
         _content = _arg_1.getContent() as MovieClip;
      ...
      private static function onPlayComHandler(_arg_1:Event) : void
      {
      ...
      private static function onAiCloseHandler(_arg_1:MouseEvent) : void
      {
         _content["closeBtn"].removeEventListener(MouseEvent.CLICK,onAiCloseHandler);
         _content.removeEventListener(Event.ENTER_FRAME,onPlayComHandler);
         DisplayUtil.removeForParent(_content);
      ...
      private static function onAppOneOpenHandler(_arg_1:Event) : void
      {
         hide1();
         showLoginTipTwo();
      }
      
      private static function onAppOneCloseHandler(_arg_1:Event) : void
      {
         hide1();
      ...
      private static function onAppTwoOpenHandler(_arg_1:Event) : void
      {
         hide2();
         playAnimation(Ai_2_Str);
      }
      
      private static function onAppTwoCloseHandler(_arg_1:Event) : void
      {
         hide2();
         NewNpcDiaDialog.show(DIA_1_A,handler_A,comStep);
      }
      ...
      private static function comStep() : void
      {
         ...
         mc["mc1"].visible = true;
         mc["mc1"].play();
         mc.addEventListener(MouseEvent.CLICK,function(_arg_1:MouseEvent):void
         {
